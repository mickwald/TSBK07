...
GLfloat calcHeight(GLfloat in_x, GLfloat in_z, int zLen, GLfloat *vertex){
	...
	if(quad_x + quad_z <= 1){
		...
		return y_o + (((y_o - y_x)*quad_x + (y_o - y_z) * quad_z) / 2);
		
	} else {
		y_xz = vertex[((x+1) + (z+1) * zLen)*3 + 1]; 
		y_x = vertex[((x+1) + z * zLen)*3 + 1];
		y_z = vertex[(x + (z+1) * zLen)*3 + 1];
		return y_xz + (((y_xz - y_x)*quad_x + (y_xz - y_z) * quad_z) / 2);
	}
}
...
void init(void)
{
	...
	LoadTGATextureSimple("Grass_tile_B_diffuse.tga", &tex1);
